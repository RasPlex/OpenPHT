...
CStdString CSysInfoJob::GetCPUFreqInfo()
{
  ...
  strCPUFreq.Format("%4.0f MHz", CPUFreq);
  return strCPUFreq;
}
...
const CStdString& CSysInfo::GetAppName(void)
{
#ifdef TARGET_RASPBERRY_PI
  static const CStdString appName("RasPlex");
#elif defined (TARGET_OPENELEC)
  static const CStdString appName("OpenPHT-Embedded");
#else
  static const CStdString appName("OpenPHT");
#endif
...
CStdString CSysInfo::GetKernelName(bool emptyIfUnknown /*= false*/)
{
...
CStdString CSysInfo::GetKernelVersionFull(void)
{
...
CStdString CSysInfo::GetKernelVersion(void)
{
...
CStdString CSysInfo::GetOsName(bool emptyIfUnknown /* = false*/)
{
...
CStdString CSysInfo::GetOsVersion(void)
{
...
CStdString CSysInfo::GetOsPrettyNameWithVersion(void)
{
...
CStdString CSysInfo::GetManufacturerName(void)
{
  static CStdString manufName;
  ...
  if (!inited)
  {
    ...
    manufName = GetDarwinManufacturer();
...
CStdString CSysInfo::GetModelName(void)
{
  static CStdString modelName;
  static bool inited = false;
  if (!inited)
  {
#if defined(TARGET_ANDROID)
    ...
    {
      char *model = new char[nameLen + 64];
      if (sysctlbyname("hw.model", model, &nameLen, NULL, 0) == 0 && nameLen > 1)
        modelName.assign(model, nameLen - 1); // assign exactly 'nameLen-1' characters to 'modelName'
      delete[] model;
    }
#elif defined(TARGET_WINDOWS)
    HKEY hKey;
...
const CStdString& CSysInfo::GetKernelCpuFamily(void)
{
  static CStdString kernelCpuFamily;
...
CStdString CSysInfo::GetUserAgent()
{
  ...
  result = GetAppName() + "/" + g_infoManager.GetLabel(SYSTEM_BUILD_VERSION_SHORT) + " (";
#if defined(TARGET_WINDOWS)
  ...
  CStdString linuxOSName(GetOsName(true));
