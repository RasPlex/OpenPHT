...
{
  m_settings.device = g_guiSettings.GetString("audiooutput.audiodevice");
  m_settings.passthoughdevice = g_guiSettings.GetString("audiooutput.passthroughdevice");

  m_settings.config = g_guiSettings.GetInt("audiooutput.config");
  m_settings.channels = (m_sink.GetDeviceType(m_settings.device) == AE_DEVTYPE_IEC958) ? AE_CH_LAYOUT_2_0 : g_guiSettings.GetInt("audiooutput.channels");
  m_settings.samplerate = g_guiSettings.GetInt("audiooutput.samplerate");

  m_settings.stereoupmix = IsSettingVisible("audiooutput.stereoupmix") ? g_guiSettings.GetBool("audiooutput.stereoupmix") : false;
  m_settings.normalizelevels = !g_guiSettings.GetBool("audiooutput.maintainoriginalvolume");
  m_settings.guisoundmode = g_guiSettings.GetInt("audiooutput.guisoundmode");

  m_settings.passthrough = m_settings.config == AE_CONFIG_FIXED ? false : g_guiSettings.GetBool("audiooutput.passthrough");
  if (!m_sink.HasPassthroughDevice())
    m_settings.passthrough = false;
  m_settings.ac3passthrough = g_guiSettings.GetBool("audiooutput.ac3passthrough");
  m_settings.ac3transcode = g_guiSettings.GetBool("audiooutput.ac3transcode");
  m_settings.eac3passthrough = g_guiSettings.GetBool("audiooutput.eac3passthrough");
  m_settings.truehdpassthrough = g_guiSettings.GetBool("audiooutput.truehdpassthrough");
  m_settings.dtspassthrough = g_guiSettings.GetBool("audiooutput.dtspassthrough");
  m_settings.dtshdpassthrough = g_guiSettings.GetBool("audiooutput.dtshdpassthrough");

  m_settings.resampleQuality = static_cast<AEQuality>(g_guiSettings.GetInt("audiooutput.processquality"));
}
...
{
  if (!m_sink.SupportsFormat(g_guiSettings.GetString("audiooutput.passthroughdevice"), format, samplerate))
    return false;
...
{
  std::string device = g_guiSettings.GetString("audiooutput.audiodevice");
  int numChannels = (m_sink.GetDeviceType(device) == AE_DEVTYPE_IEC958) ? AE_CH_LAYOUT_2_0 : g_guiSettings.GetInt("audiooutput.channels");
  bool passthrough = g_guiSettings.GetInt("audiooutput.config") == AE_CONFIG_FIXED ? false : g_guiSettings.GetBool("audiooutput.passthrough");
  return numChannels == AE_CH_LAYOUT_2_0 && ! (passthrough &&
    g_guiSettings.GetBool("audiooutput.ac3passthrough") &&
    g_guiSettings.GetBool("audiooutput.ac3transcode"));
}
...
bool CActiveAE::IsSettingVisible(const std::string &settingId)
{
  ...
  {
    if (m_sink.GetDeviceType(g_guiSettings.GetString("audiooutput.audiodevice")) == AE_DEVTYPE_IEC958)
      return true;
  ...
  {
    if (m_sink.HasPassthroughDevice() && g_guiSettings.GetInt("audiooutput.config") != AE_CONFIG_FIXED)
      return true;
  }
  else if (settingId == "audiooutput.truehdpassthrough")
  {
    if (m_sink.SupportsFormat(g_guiSettings.GetString("audiooutput.passthroughdevice"), AE_FMT_TRUEHD, 192000) &&
        g_guiSettings.GetInt("audiooutput.config") != AE_CONFIG_FIXED)
      return true;
  }
  else if (settingId == "audiooutput.dtshdpassthrough")
  {
    if (m_sink.SupportsFormat(g_guiSettings.GetString("audiooutput.passthroughdevice"), AE_FMT_DTSHD, 192000) &&
        g_guiSettings.GetInt("audiooutput.config") != AE_CONFIG_FIXED)
      return true;
  }
  else if (settingId == "audiooutput.eac3passthrough")
  {
    if (m_sink.SupportsFormat(g_guiSettings.GetString("audiooutput.passthroughdevice"), AE_FMT_EAC3, 192000) &&
        g_guiSettings.GetInt("audiooutput.config") != AE_CONFIG_FIXED)
      return true;
  }
  ...
  {
    if (m_sink.HasPassthroughDevice() ||
        g_guiSettings.GetInt("audiooutput.channels") > AE_CH_LAYOUT_2_0)
    return true;
  }
  else if (settingId == "audiooutput.ac3transcode")
  {
    if (m_sink.HasPassthroughDevice() &&
        g_guiSettings.GetBool("audiooutput.ac3passthrough") &&
        g_guiSettings.GetInt("audiooutput.config") != AE_CONFIG_FIXED &&
        (g_guiSettings.GetInt("audiooutput.channels") <= AE_CH_LAYOUT_2_0 || m_sink.GetDeviceType(g_guiSettings.GetString("audiooutput.audiodevice")) == AE_DEVTYPE_IEC958))
      return true;
  }
  return false;
}
