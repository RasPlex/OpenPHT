...
{
  m_userOutput = g_guiSettings.GetString("videoscreen.monitor");
  XOutput *out = NULL;
...
void CWinSystemX11::UpdateResolutions()
{
  ...
  bool switchOnOff = g_guiSettings.GetBool("videoscreen.blankdisplays");
  m_userOutput = g_guiSettings.GetString("videoscreen.monitor");
  if (m_userOutput.compare("Default") == 0)
    switchOnOff = false;
  ...
  if(g_xrandr.Query(true, !switchOnOff))
  {
    ...
    g_settings.m_ResInfo[RES_DESKTOP].strOutput = m_userOutput;
  }
  else
  {
...
void CWinSystemX11::RecreateWindow()
{
  ...
  {
    res = g_settings.m_ResInfo[i];
    if (g_settings.m_ResInfo[i].strId.Equals(mode.id))
    {
      found = true;
      break;
    }
...
bool CWinSystemX11::SetWindow(int width, int height, bool fullscreen, const std::string &output, int *winstate)
{
  ...
  if (changeWindow)
  {
    m_icon = None;
    {
      ...
      std::string titleString = PLEX_TARGET_NAME;
