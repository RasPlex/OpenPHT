...
void GUIFontManager::LoadFonts(const CStdString& strFontSet)
{
  ...
  while (pChild)
  {
    ...
    if (idAttr)
    {
      ...
      if (StringUtils::EqualsNoCase(strFontSet, idAttr))
      {
...
void GUIFontManager::LoadFonts(const TiXmlNode* fontNode)
{
  while (fontNode)
  {
    ...
    if (!fontName.empty() && URIUtils::GetExtension(fileName).Equals(".ttf"))
    {
...
void GUIFontManager::GetStyle(const TiXmlNode *fontNode, int &iStyle)
{
  ...
  {
    std::vector<std::string> styles = StringUtils::Split(style, " ");
