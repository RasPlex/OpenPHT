...
class CStereoscopicsManager : public IMsgTargetCallback
{
public:
  ...
  CStdString DetectStereoModeByString(const CStdString &needle);
  RENDER_STEREO_MODE GetStereoModeByUserChoice(const CStdString &heading = "");
  RENDER_STEREO_MODE GetStereoModeOfPlayingVideo(void);
  int GetLabelIdForStereoMode(const RENDER_STEREO_MODE &mode) const;
  const CStdString &GetLabelForStereoMode(const RENDER_STEREO_MODE &mode) const;
  RENDER_STEREO_MODE GetPreferredPlaybackMode(void);
  int ConvertVideoToGuiStereoMode(const CStdString &mode);
  /**
   ...
   */
  int ConvertStringToGuiStereoMode(const CStdString &mode);
   ...
   */
  CAction ConvertActionCommandToAction(const CStdString &command, const CStdString &parameter);
  CStdString NormalizeStereoMode(const CStdString &mode);
